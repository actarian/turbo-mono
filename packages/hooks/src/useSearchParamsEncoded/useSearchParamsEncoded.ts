...
export function decode(encoded: string | null | undefined, key?: string): IParams {
  ...
  // console.log('useSearchParamsEncoded.decode', decoded);
  if (key && decoded) {
    decoded = decoded[key];
  }
  return decoded || null;
...
function encode(params: IParams, value: IParams, key?: string) {
  ...
  // console.log('useSearchParamsEncoded.encode', params);
...
export function getSearchParamsEncoded(url: string, key?: string): {} | null {
  const components = url.split('?');
  const search = components[1] || '';
  const searchParams = new URLSearchParams(search);
  const params = searchParams.get('params');
  const value = params ? decode(params, key) : null;
  // console.log('getSearchParamsEncoded', value, params, search);
  return value;
}
...
export function useSearchParamsEncoded(key?: string): IUseSearchParamsResult {
  ...
  const initialValue = useMemo(() => {
    const value = getSearchParamsEncoded(router.asPath, key);
    return value;
  }, [key, router.asPath]);

  const params = initialValue;
  // console.log('useSearchParamsEncoded', params);

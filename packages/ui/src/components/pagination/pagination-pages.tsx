...
const PaginationPages: React.FC<Props> = ({ limit, count, current, setPage }) => {
  ...
  const renderMiddleItems = pagesArray.map((_, index) => {
    ...
    return (
      <PaginationItem key={`pagination-middle-${index}`} active={index + 1 === middleNumber} onClick={() => setPage(value)}>
        {value}
      </PaginationItem>
    );
  });
    ...
    return (
      <>
        {[...new Array(count)].map((_, index) => {
          const value = index + 1;
          return (
            <PaginationItem key={`pagination-item-${value}`} active={value === current} onClick={() => setPage(value)}>
              {value}
            </PaginationItem>
          );
        })}
